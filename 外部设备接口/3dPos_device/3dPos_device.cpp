// 3dPos_device.cpp: 定义 DLL 的初始化例程。
//
...
BEGIN_MESSAGE_MAP(CMy3dPosdeviceApp, CWinApp)
END_MESSAGE_MAP()
...
CMy3dPosdeviceApp::CMy3dPosdeviceApp()
{
	// TODO:  在此处添加构造代码，
...
// 唯一的 CMy3dPosdeviceApp 对象

CMy3dPosdeviceApp theApp;


// CMy3dPosdeviceApp 初始化

BOOL CMy3dPosdeviceApp::InitInstance()
{
...
bool MoveDLLIni(void* pmv, void*& p2)
{
	...
	for (int i = 0; i < dlnum; i++)
	{
		...
		if (!pa.m_iniSucc)
		{
			string data = ((CDevice*)p2)->m_cfgDir + " 3d模板加载失败！";
	...
	if (NULL != mb)
	{
		if (NULL == p2)
		{
			...
			for (int i = 0; i < dlnum; i++)
			{
				...
				if (!pa.m_iniSucc)
				{
					data = ((CDevice*)p2)->m_cfgDir + " 3d模板加载失败！";
					AfxMessageBox(data.c_str());
					delete p2;
					p2 = NULL;
					return false;
