...
void ST_MYDISP::IPRunCamCaliInner()
{
	...
	if(id > -1)
	{
		...
		if(-1 != ((checkPatch*)checkPatch::S_dlg.m_pCheck)->CameraDrv::grapImage(id-1,m))
			Image = Mat2HObject(m);
		else
			return;
...
void ST_MYDISP::DisCamliInnerOnePicture(HalconCpp::HImage * Image, HalconCpp::HXLDCont*  Contours,int prID)
{
	try
	{
		...
		// 设置标定初始参数
		CString calimodel = (*om)["CamCalibrateModel"];
	...
	HalconCpp::HTuple & hv_TmpCtrl_FindCalObjParValues)
{
	...
	hv_StartParameters[8] = this->GetWidth(om) / 2;
	hv_StartParameters[9] = this->GetHeight(om) / 2;
	hv_StartParameters[10] = this->GetWidth(om);
	hv_StartParameters[11] = this->GetHeight(om);
	...
	double gap_tolerance = (*om)["标定间距容许因子"];
	double alpha = (*om)["标定平滑alpha"];
	double max_diam_marks = (*om)["标定最大标志点直径"];
