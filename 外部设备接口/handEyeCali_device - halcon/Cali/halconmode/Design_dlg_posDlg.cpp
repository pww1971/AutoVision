...
BOOL CDesign_dlg_posDlg::OnInitDialog()
{
	...
	ScreenToClient(&rect);
	if (!m_wndProperties.Create("属性", this, CRect(rect.right + 2, rect.top, rect.right + 250, rect.bottom), TRUE, ID_VIEW_PROPERTIESWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_RIGHT | CBRS_FLOAT_MULTI))
	{
		TRACE0("未能创建“属性”窗口\n");
		return FALSE; // 未能创建
	}
...
void CDesign_dlg_posDlg::OnBnClickedBtCamOneShot()
{
	...
	if (id > -1 )
	{
		...
		{
			UCI::OPTIONS["图像宽度"] = m.cols;
			UCI::OPTIONS["图像高度"] = m.rows;
			UCI::OPTIONS["图像宽长比"] = (double)m.rows / m.cols;
