...
BOOL ST_MYDISP::LoadCaliPoseAndParamter(string fileAdd,int prID)
{
	...
	//	sprintf(sz,"%d\\pos\\",prID);
		...
		// 调整坐标原点 
		double orgX = (*om)["标定原点 X 坐标"];
		double orgY = (*om)["标定原点 Y 坐标"];
		double orgZ = (*om)["标定原点 Z 坐标"];

		double angleX = (*om)["标定 X 轴 角度"];
		double angleY = (*om)["标定 Y 轴 角度"];
		double angleZ = (*om)["标定 Z 轴 角度"];
		...
		double TmpCtrl_RectificationWidth = (*om)["映射宽度"];
		...
		// 建立 ImagMap
		this->hv_Cali->ho_RectMap.GenImageToWorldPlaneMap(this->hv_Cali->hv_CamParameters,
...
BOOL ST_MYDISP::OnDisMeasureOnePic(UCI::OptionsMap * camo, HImage * img)
{
	try {
		...
		if (isUseStorePic) {   // 加载一个测试图			
			...
			int prID = UCI::OPTIONS["项目编号"];
			cv::Mat m;
			if (-1==((checkPatch*)checkPatch::S_dlg.m_pCheck)->grapImage(prID,m))
			{
