...
bool TotDLLMove(void* pmv,void*& p2,string& data)
{
	...
	if (NULL != mb)
	{
		...
		if (vct[0] == "check" && vct.size()==3)//拍照检测 
		{
			...
			{
				if (!myserver->sendAndWaitReturnMsg(e_CAM_CHECK,pmsg))
				{
					delete pmsg;
					data = "10秒内没收到客户端拍照相机返回信息";
					return false;//说明拍照失败
				}
				delete pmsg;
			}
			...
			{
				if(myserver!=NULL && myserver->m_tcpNum > 0)
				{
					u_long ulLongTemp[2] = {htonl(e_CAM_OFF),htonl(-2)};	//清除副机缓存	
					myserver->SendPackage2All((char*)ulLongTemp,sizeof(long)*2);
				}
				//推人后台就返回
