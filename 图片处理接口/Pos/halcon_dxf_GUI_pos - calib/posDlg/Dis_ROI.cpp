...
{
	try {
		HObject  ho_DXFModelContours, ho_Contours, ho_ObjectSelected, ho_ModelWorld;
		HTuple hv_DXFStatus, hv_HomMat2DIdentity, hv_HomMat2DScale;
		HTuple  hv_HomMat3D, hv_NumberContour;
		HTuple  hv_I, hv_Y, hv_X, hv_Z, hv_Xc, hv_Yc, hv_Zc, hv_R, hv_C;

		hv_Model->hv_CamParam.Clear();
		hv_Model->hv_CamParam[0] = 0.06;
		hv_Model->hv_CamParam[1] = 0;
		hv_Model->hv_CamParam[2] = 8.5e-6;
		hv_Model->hv_CamParam[3] = 8.5e-6;
		hv_Model->hv_CamParam.Append(640 / 2);
		hv_Model->hv_CamParam.Append(480 / 2);
		hv_Model->hv_CamParam.Append(640);
		hv_Model->hv_CamParam.Append(480);

		hv_Model->hv_ReferencePose.Clear();
		hv_Model->hv_ReferencePose[0] = 0;
		hv_Model->hv_ReferencePose[1] = 0;
		hv_Model->hv_ReferencePose[2] = 10;
		hv_Model->hv_ReferencePose[3] = 0;
		hv_Model->hv_ReferencePose[4] = 0;
		hv_Model->hv_ReferencePose[5] = 0;
		hv_Model->hv_ReferencePose[6] = 0;
		...
		HTuple  hv_Row1, hv_Column1, hv_Row2, hv_Column2;
		{
			...
			HomMat2dScale(hv_HomMat2DIdentity, 
				dscale, dscale,(hv_Row2.TupleMax() - hv_Row1.TupleMin()) / 2, (hv_Column2.TupleMax() - hv_Column1.TupleMin()) / 2,
				&hv_HomMat2DScale);
		...
		{
			PoseToHomMat3d(hv_Model->hv_ReferencePose, &hv_HomMat3D);
			GenEmptyObj(&ho_Contours);
			CountObj(ho_DXFModelContours, &hv_NumberContour);

			HTuple end_val46 = hv_NumberContour;
			HTuple step_val46 = 1;
			for (hv_I = 1; hv_I.Continue(end_val46, step_val46); hv_I += step_val46)
			{
				SelectObj(ho_DXFModelContours, &ho_ObjectSelected, hv_I);
				GetContourXld(ho_ObjectSelected, &hv_Y, &hv_X);
				hv_Z = HTuple(hv_X.TupleLength(), 0.0);
				//Transform the metric model into world coordinates.
				AffineTransPoint3d(hv_HomMat3D, hv_X, hv_Y, hv_Z, &hv_Xc, &hv_Yc, &hv_Zc);
				//Project the 3D points of the model, which are available
				//as world coordinates, into the image coordinate system.
				Project3dPoint(hv_Xc, hv_Yc, hv_Zc, hv_Model->hv_CamParam, &hv_R, &hv_C);
				GenContourPolygonXld(&ho_ModelWorld, hv_R, hv_C);
				ConcatObj(ho_Contours, ho_ModelWorld, &ho_Contours);
			}
		...
		CreatePlanarCalibDeformableModelXld(
			ho_DXFModelContours,
			hv_Model->hv_CamParam, hv_Model->hv_ReferencePose,//相机的 内参和姿态
			CreatNumLevels,
			CreatAngleStart,
			CreatAngleExtend,
			AngleStep,
		...
		if (Score.Length() > 0)
		{
			...
			for (hv_J = 0; hv_J.Continue(end_val81, step_val81); hv_J += step_val81)
			{
				...
				TupleSelectRange(hv_Poses, hv_J * 7, ((hv_J + 1) * 7) - 1, &hv_Pose);
				//Derive the transformation matrix for the detected
				//model from its 3D pose.
				PoseToHomMat3d(hv_Pose, &hv_HomMat3D);
				GenEmptyObj(&ho_Contours);
				CountObj(hv_Model->ho_ModelContours, &hv_NumberContour);
				{
					HTuple end_val89 = hv_NumberContour;
					HTuple step_val89 = 1;
					HTuple hv_K;
					for (hv_K = 1; hv_K.Continue(end_val89, step_val89); hv_K += step_val89)
					{
						SelectObj(hv_Model->ho_ModelContours, &ho_ObjectSelected, hv_K);
						GetContourXld(ho_ObjectSelected, &hv_Y, &hv_X);
			...
			//从图像中添加极性
			SetPlanarCalibDeformableModelMetric(m_sIPResult.result_img, hv_Model->m_dxfModel, hv_Pose, "use_polarity");
			//设置原点
		//	SetDeformableModelOrigin(hv_Model->m_dxfModel, 0.010, 0.0425);
			GetDeformableModelContours(&hv_Model->ho_ModelContours, hv_Model->m_dxfModel, 1);

			this->m_pMHWinMain->DispObj(ho_Contours);
			disp_3d_coord_system(this->m_pMHWinMain, hv_Model->hv_CamParam, hv_Pose, 0.05);
		}
	}
	catch (HalconCpp::HException& except) {
		AfxMessageBox(except.ErrorText().Text());
