...
//		if (isHomMat2DOK == FALSE) return FALSE;
		...
		(*o)["RES_Circle_PointOrders"] = (CString)hv_PointOrders[0].S().Text();
		(*o)["RES_Circle_Type"] = (CString)hv_ArcType[0].S().Text();
	}
	catch (HException& except) {
		AfxMessageBox(except.ErrorText().Text());
		return FALSE;
...
BOOL ST_MYDISP::MeasureOneROI(UCI::OptionsMap * o, HalconCpp::HImage & hImg, HalconCpp::HXLDCont &)
{
	try {
		...
		//			AfxMessageBox("MeasureOneFUNCTION()： 加载映射图像失败！");
		//			return FALSE;
		//		}
		//	}*/
		//	// 校正图像
...
BOOL ST_MYDISP::MeasureOneMATCH(UCI::OptionsMap* o, HImage& hImg, HXLDCont& Cont)
{
	try {
		...
		if (Score.Length() > 0)
		{
			...
			HObject ho_Contours, ho_ObjectSelected, ho_ModelWorld;
			for (hv_J = 0; hv_J.Continue(end_val81, step_val81); hv_J += step_val81)
			{
				...
				CountObj(hv_Model->ho_ModelContours, &hv_NumberContour);
				{
					...
					Project3dPoint(hv_Xc, hv_Yc, hv_Zc, hv_Model->hv_CamParam, &Row, &Column);
					...
					(*o)["RES_FindRow"] = Row.TupleMean();
					(*o)["RES_FindColumn"] = Column.TupleMean();
					(*o)["RES_FindAngle"] = Angle.TupleDeg();
					(*o)["检测相似度"] = Score;
