	...
	try
	{
		...
		if ("" != szVal)
		{
			...
			for (int i = 1; i < count; i++)
			{
				...
				string camstr = m_cfgDir + "\\model" + sz + ".cal";
				ReadCamPar(camstr.c_str(), &model->hv_CamParam);
				string posestr = m_cfgDir + "\\model" + sz + ".pose";
				ReadTuple( posestr.c_str(), &model->hv_ReferencePose);
...
void checkPatch::writeMode(string file)
{
	...
	while (pos != NULL) {
		...
		string camstr = m_cfgDir + "\\model" + sz + ".cal";
		remove(camstr.c_str());
		WriteCamPar(model->hv_CamParam,camstr.c_str());
		string posestr = m_cfgDir + "\\model" + sz + ".pose";
		remove(posestr.c_str());
		WriteTuple(model->hv_ReferencePose, posestr.c_str());
		i++;
	}
}
...
bool checkPatch::checkLocateCheck(cv::Mat& src, vector<S_TpltData*>& vctData, CTpltParam& Pa)
{
	...
	while (pos != NULL) {
		...
		try
		{
			...
			HTuple hv_Pose, hv_ObjectModel3DRigidTrans, hv_ParamValue;
			...
			if (NumMatchesLimit > len) {
				...
				HTuple  hv_I, hv_Y, hv_X, hv_Z, hv_Xc, hv_Yc, hv_Zc, hv_R, hv_C, Angle;
				HObject  ho_ObjectSelected;

				char sz[128];
				for (int a = 0; a < len; a++)
				{
					hv_Pose = hv_Poses.TupleSelectRange(a * 7, (a * 7) + 6);
					PoseToHomMat3d(hv_Pose, &hv_HomMat3D);
					AffineTransPoint3d(hv_HomMat3D, hv_Model->hv_CentX, hv_Model->hv_CentY, HTuple(0).Append(0), &hv_Xc, &hv_Yc, &hv_Zc);
					...
					vctData[a]->m_roiRect.angle = Angle.TupleDeg().D();
					...
					vctData[a]->m_roiRect.center.x = hv_C.TupleMean().D();
					vctData[a]->m_roiRect.center.y = hv_R.TupleMean().D();
...
bool checkPatch::creatDxfModel(string dxffile, CTpltParam& pa,cv::RotatedRect& rrt)
{
	try {
		...
		rrt.angle = 0;
		rrt.size.width = (hv_Model->hv_CentX[0] - hv_Model->hv_CentX[1])*700 ;
		rrt.size.height = (hv_Model->hv_CentY[0] - hv_Model->hv_CentY[1])*700;
		rrt.center.x = 0;
		rrt.center.y = 0;
