...
void dev_display_model_output_image(HTuple hv_ExampleInternals);
void dev_display_ok_nok(HTuple hv_Areas, HTuple hv_WindowHandleImage);
void dev_display_pills_example_dataset_preview();
void dev_display_preprocessed_image(HTuple hv_ExampleInternals);
void dev_display_raw_image(HTuple hv_WindowHandleImages);
void dev_display_screen_device(HTuple hv_ExampleInternals, HTuple hv_DLDevice);
void dev_display_screen_example_images(HTuple hv_ExampleInternals, HTuple hv_UsePretrainedModel);
...
void dev_display_screen_inference_step_2(HTuple hv_ExampleInternals);
void dev_display_screen_inference_step_3(HTuple hv_ExampleInternals);
void dev_display_screen_inference_step_4(HTuple hv_ExampleInternals);
void dev_display_screen_introduction(HTuple hv_ExampleInternals);
void dev_display_screen_ready_to_infer(HTuple hv_ExampleInternals);
void dev_display_segmented_regions(HTuple hv_ExampleInternals);
...
void get_inference_images(HTuple hv_ImageDir, HTuple* hv_ImageFiles);
...
void dev_display_example_reset_windows(HTuple hv_ExampleInternals)
{
    ...
    HTuple  hv_WindowHandleImages, hv_WindowLegendNeeded, hv_WindowHandleLegend;
    ...
    try
    {
        ...
        if (HDevWindowStack::IsOpen())
            SetWindowExtents(HDevWindowStack::GetActive(), 360, 0, 500, 500);
        if (HDevWindowStack::IsOpen())
...
void dev_display_model_output_image(HTuple hv_ExampleInternals)
{
    ...
    // Local control variables
    HTuple  hv_PreprocessParamFileName, hv_DLPreprocessParam;

    //This procedure visualizes the output of a segmentation model,
    //by displaying a segmentation image.
    //
    //Read the preprocessed image.
    ReadImage(&ho_Image, "pill/ginseng/contamination/pill_ginseng_contamination_007.png");
    ReadImage(&ho_SegImage, "labels/pill/ginseng/contamination/pill_ginseng_contamination_007_gt.png");
    //Preprocess the segmentation image.
    GetDictTuple(hv_ExampleInternals, "preprocess_param_file_name", &hv_PreprocessParamFileName);
    ReadDict(hv_PreprocessParamFileName, HTuple(), HTuple(), &hv_DLPreprocessParam);
    preprocess_dl_model_segmentations(ho_Image, ho_SegImage, &ho_SegImage, hv_DLPreprocessParam);
    //
    //Display segmentation image.
    if (HDevWindowStack::IsOpen())
        DispObj(ho_SegImage, HDevWindowStack::GetActive());
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), "Segmentation image \n(output of 'apply_dl_model')",
            "window", "top", "left", "black", HTuple(), HTuple());
    //
    //Write segmentation image to file.
    SetDictObject(ho_SegImage, hv_ExampleInternals, "segmentation_image");
    //
    return;
}
...
void dev_display_ok_nok(HTuple hv_Areas, HTuple hv_WindowHandleImage)
{
    ...
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
            (HTuple("box_color").Append("shadow")), hv_BoxColor.TupleConcat("false"));
    ...
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), "Press Run (F5) to continue", "window",
            "bottom", "left", "black", "box", "true");
    FlushBuffer(hv_WindowHandleImage);
    return;
}
...
void dev_display_pills_example_dataset_preview()
{
    ...
    // Local iconic variables
    HObject  ho_Image, ho_TiledImage, ho_ImageR, ho_ImageG;
    HObject  ho_ImageB, ho_ImageRG, ho_ImageRGB;

    // Local control variables
    HTuple  hv_GinsengPath, hv_MagnesiumPath, hv_MintPath;
    HTuple  hv_Width, hv_Height, hv_Width1, hv_Height1, hv_Exception;
    HTuple  hv_ErrorAndAdviceText;
    ...
    try
    {
        ...
        hv_GinsengPath = HTuple("pill/ginseng/") + ((HTuple("good/pill_ginseng_good_001").Append("contamination/pill_ginseng_contamination_004")).Append("crack/pill_ginseng_crack_001"));
        hv_MagnesiumPath = HTuple("pill/magnesium/") + ((HTuple("good/pill_magnesium_good_001").Append("contamination/pill_magnesium_contamination_001")).Append("crack/pill_magnesium_crack_001"));
        hv_MintPath = HTuple("pill/mint/") + ((HTuple("good/pill_mint_good_001").Append("contamination/pill_mint_contamination_001")).Append("crack/pill_mint_crack_009"));
        ReadImage(&ho_Image, (hv_GinsengPath.TupleConcat(hv_MagnesiumPath)).TupleConcat(hv_MintPath));
        TileImages(ho_Image, &ho_TiledImage, 3, "horizontal");
        //Generate background image.
        GetImageSize(ho_TiledImage, &hv_Width, &hv_Height);
        GenImageProto(ho_TiledImage, &ho_ImageR, 18);
        GenImageProto(ho_TiledImage, &ho_ImageG, 22);
        GenImageProto(ho_TiledImage, &ho_ImageB, 28);
        AppendChannel(ho_ImageR, ho_ImageG, &ho_ImageRG);
        AppendChannel(ho_ImageRG, ho_ImageB, &ho_ImageRGB);
        //Display the background and the images.
        if (HDevWindowStack::IsOpen())
            SetWindowExtents(HDevWindowStack::GetActive(), 360, 0, 800, 400);
        GetImageSize(ho_ImageRGB, &hv_Width1, &hv_Height1);
        if (HDevWindowStack::IsOpen())
            SetPart(HDevWindowStack::GetActive(), 0, 0, hv_Height1, hv_Width1);
        if (HDevWindowStack::IsOpen())
            DispObj(ho_ImageRGB, HDevWindowStack::GetActive());
        if (HDevWindowStack::IsOpen())
    ...
    catch (HException& HDevExpDefaultException)
    {
        ...
        hv_ErrorAndAdviceText = hv_ErrorAndAdviceText.TupleConcat("");
        hv_ErrorAndAdviceText = hv_ErrorAndAdviceText.TupleConcat("These images are part of a separate installer. Please");
        hv_ErrorAndAdviceText = hv_ErrorAndAdviceText.TupleConcat("refer to the Installation Guide for more information on");
        hv_ErrorAndAdviceText = hv_ErrorAndAdviceText.TupleConcat("this topic!");
        if (HDevWindowStack::IsOpen())
            ClearWindow(HDevWindowStack::GetActive());
        if (HDevWindowStack::IsOpen())
            DispText(HDevWindowStack::GetActive(), hv_ErrorAndAdviceText, "window", "center",
                "left", "red", HTuple(), HTuple());
    }
    return;
}
...
void dev_display_preprocessed_image(HTuple hv_ExampleInternals)
{
    ...
    ReadDict(hv_PreprocessParamFileName, HTuple(), HTuple(), &hv_DLPreprocessParam);
    //
    preprocess_dl_model_images(ho_Image, &ho_Image, hv_DLPreprocessParam);
    SetDictObject(ho_Image, hv_ExampleInternals, "preprocessed_image");
    //
    //Display preprocessed image.
    if (HDevWindowStack::IsOpen())
        SetPart(HDevWindowStack::GetActive(), 0, 0, 400, 400);
    if (HDevWindowStack::IsOpen())
        DispObj(ho_Image, HDevWindowStack::GetActive());
    if (HDevWindowStack::IsOpen())
...
void dev_display_raw_image(HTuple hv_WindowHandleImages)
{
    ...
    HTuple  hv_Width1, hv_Height1, hv_ZoomFactor;
    ...
    GetWindowExtents(hv_WindowHandleImages, &hv_Row, &hv_Column, &hv_Width1, &hv_Height1);
    //
    if (0 != (int(hv_Height < hv_Width)))
    {
    ...
    ZoomImageFactor(ho_Image, &ho_Image, hv_ZoomFactor, hv_ZoomFactor, "bilinear");
    //
    if (HDevWindowStack::IsOpen())
        SetWindowExtents(HDevWindowStack::GetActive(), 360, 0, hv_ZoomFactor * hv_Width,
...
void dev_display_screen_example_images(HTuple hv_ExampleInternals, HTuple hv_UsePretrainedModel)
{
    ...
    HTuple  hv_ExampleDataDir, hv_ExampleDataDirExists, hv_PreprocessParamFileName;
    HTuple  hv_DataDirectory, hv_PreprocessParamsExist, hv_RetrainedModelFileName;
    HTuple  hv_ModelExists, hv_ExceptionText, hv_NumMissing;
    ...
    FileExists(hv_ExampleDataDir, &hv_ExampleDataDirExists);
    //
    ...
    FileExists(hv_PreprocessParamFileName, &hv_PreprocessParamsExist);
    //
    ...
    if (0 != hv_UsePretrainedModel)
    {
        if (0 != (HTuple(hv_ModelExists.TupleNot()).TupleOr(hv_PreprocessParamsExist.TupleNot())))
        {
            GetDictTuple(hv_ExampleInternals, "window_text", &hv_WindowHandleText);
            HDevWindowStack::SetActive(hv_WindowHandleText);
            //
            hv_ExceptionText = "To run this example you need the output of:";
            hv_ExceptionText[hv_ExceptionText.TupleLength()] = HTuple(" - Deep learning installer, see Installation Guide.");
            hv_ExceptionText[hv_ExceptionText.TupleLength()] = "";
            //Display warning.
            set_display_font(hv_WindowHandleText, 20, "mono", "true", "false");
            if (HDevWindowStack::IsOpen())
                DispText(HDevWindowStack::GetActive(), hv_ExceptionText, "window", "top",
                    "left", "red", "box", "true");
            return;
        }
    }
    ...
    {
        if (0 != (HTuple(HTuple(hv_ExampleDataDirExists.TupleNot()).TupleOr(hv_ModelExists.TupleNot())).TupleOr(hv_PreprocessParamsExist.TupleNot())))
        {
            hv_ExceptionText = "To run this example you need the output of:";
            hv_NumMissing = 0;
            if (0 != (HTuple(hv_PreprocessParamsExist.TupleNot()).TupleOr(hv_ExampleDataDirExists.TupleNot())))
            {
                hv_ExceptionText[hv_ExceptionText.TupleLength()] = " - 'segmentation_pill_defects_deep_learning_1_preprocess.hdev'";
                hv_NumMissing += 1;
            }
            if (0 != (hv_ModelExists.TupleNot()))
            {
                hv_ExceptionText[hv_ExceptionText.TupleLength()] = " - 'segmentation_pill_defects_deep_learning_2_train.hdev'";
                hv_NumMissing += 1;
            }
            hv_ExceptionText[hv_ExceptionText.TupleLength()] = "";
            if (0 != (int(hv_NumMissing == 1)))
            {
                hv_ExceptionText[hv_ExceptionText.TupleLength()] = "Please run this example first.";
            }
            else if (0 != (int(hv_NumMissing > 1)))
            {
                hv_ExceptionText[hv_ExceptionText.TupleLength()] = "Please run these examples first.";
            }
            hv_ExceptionText[HTuple::TupleGenSequence(hv_ExceptionText.TupleLength(), (hv_ExceptionText.TupleLength()) + 2, 1)] = ((HTuple("Alternatively, you can set 'UsePretrainedModel := true' ").Append("at the top of the example script to use an already trained")).Append("model shipped with the HALCON installation."));
    ...
    SetDictTuple(hv_ExampleInternals, "preprocess_param_file_name", hv_PreprocessParamFileName);
    //
    //Reset the open windows for a clean display.
    ...
    HDevWindowStack::SetActive(hv_WindowHandleText);
    //
    hv_Text = HTuple("We now have a trained segmentation model,");
    hv_Text[hv_Text.TupleLength()] = "which is ready to be applied to new images.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = HTuple("Below, you see a few example images.");
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = HTuple("These images represent newly incoming images,");
    hv_Text[hv_Text.TupleLength()] = "i.e. they are not part of the preprocessed dataset.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = HTuple("The images have to be preprocessed in the same way as the DLDataset,");
    hv_Text[hv_Text.TupleLength()] = "which was used for training.";
    //
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
            "box", "true");
...
void dev_display_screen_final(HTuple hv_ExampleInternals)
{
    ...
    hv_Text[hv_Text.TupleLength()] = "You have finished the series of examples for DL segmentation.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = "You can now train a segmentation model on your own data.";
    hv_Text[hv_Text.TupleLength()] = "";
    //
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
...
void dev_display_screen_inference_step_1(HTuple hv_ExampleInternals)
{
    ...
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = "1. Generate a DLSample for the image";
    hv_Text[hv_Text.TupleLength()] = "   using the procedure 'gen_dl_samples_from_images'.";
    //
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
            "box", "true");
...
void dev_display_screen_inference_step_2(HTuple hv_ExampleInternals)
{
    ...
    hv_Text[hv_Text.TupleLength()] = "   using the procedure 'gen_dl_samples_from_images'.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = "2. Preprocess the image to fulfill the requirements of the trained model";
    hv_Text[hv_Text.TupleLength()] = "   using the procedure 'preprocess_dl_samples'.";
    //
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
            "box", "true");
...
void dev_display_screen_inference_step_3(HTuple hv_ExampleInternals)
{
    ...
    hv_Text[hv_Text.TupleLength()] = "   using the procedure 'preprocess_dl_samples'.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = "3. Apply the model using the operator 'apply_dl_model'.";
    //
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
            "box", "true");
...
void dev_display_screen_inference_step_4(HTuple hv_ExampleInternals)
{
    ...
    hv_Text[hv_Text.TupleLength()] = "3. Apply the model using the operator 'apply_dl_model'.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = "4. Postprocess the output segmentation image";
    hv_Text[hv_Text.TupleLength()] = "   to get segmented regions for each class.";
    hv_Text[hv_Text.TupleLength()] = "";
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
            "box", "true");
...
void dev_display_screen_introduction(HTuple hv_ExampleInternals)
{
    ...
    hv_Text[hv_Text.TupleLength()] = "the workflow for DL segmentation.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = "The four parts are: ";
    hv_Text[hv_Text.TupleLength()] = "1. Dataset preprocessing.";
...
void dev_display_screen_ready_to_infer(HTuple hv_ExampleInternals)
{
    ...
    hv_Text[hv_Text.TupleLength()] = "(segmentation_pill_defects_deep_learning_2_train.hdev)";
    hv_Text[hv_Text.TupleLength()] = "to some new images using 'apply_dl_model'.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = HTuple("Additionally, we apply postprocessing steps to the");
    hv_Text[hv_Text.TupleLength()] = "output segmentation image:";
    hv_Text[hv_Text.TupleLength()] = " 1. Get a segmented region for each class.";
    hv_Text[hv_Text.TupleLength()] = " 2. Split the segmented regions into connected components.";
    hv_Text[hv_Text.TupleLength()] = " 3. Compute the area of each defect instance.";
    hv_Text[hv_Text.TupleLength()] = "";
    hv_Text[hv_Text.TupleLength()] = "The final result will be displayed for each image.";
    //
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top", "left", "black",
            "box", "true");
    ...
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), "Press Run (F5) to continue", "window",
            "bottom", "right", "black", "box", "true");
    //
    SetDictTuple(hv_ExampleInternals, "window_handles_to_close", hv_WindowHandleText);
    //
    return;
}
...
void dev_display_segmented_regions(HTuple hv_ExampleInternals)
{
    ...
    // Local control variables
    HTuple  hv_Area, hv_Row, hv_Column, hv_Info, hv_ClassIDs;
    HTuple  hv_ClassNames, hv_Sample, hv_Result, hv_GenParam;
    HTuple  hv_WindowHandleDict, hv_WindowHandleImages, hv_WindowHandleLegend;
    ...
    GetDictObject(&ho_Image, hv_ExampleInternals, "preprocessed_image");
    GetDictObject(&ho_SegImage, hv_ExampleInternals, "segmentation_image");
    //
    //Get defect region and compute its area.
    Threshold(ho_SegImage, &ho_Defect, 1, 1);
    AreaCenter(ho_Defect, &hv_Area, &hv_Row, &hv_Column);
    //
    ...
    SetDictTuple(hv_Info, "class_ids", hv_ClassIDs);
    SetDictTuple(hv_Info, "class_names", hv_ClassNames);
    //Sample.
    CreateDict(&hv_Sample);
    SetDictObject(ho_Image, hv_Sample, "image");
    //Result.
    CreateDict(&hv_Result);
    SetDictObject(ho_SegImage, hv_Result, "segmentation_image");
    //GenParam.
    CreateDict(&hv_GenParam);
    SetDictTuple(hv_GenParam, "segmentation_exclude_class_ids", 0);
    SetDictTuple(hv_GenParam, "segmentation_transparency", "80");
    SetDictTuple(hv_GenParam, "font_size", 16);
    //WindowHandleDict.
    CreateDict(&hv_WindowHandleDict);
    GetDictTuple(hv_ExampleInternals, "window_images", &hv_WindowHandleImages);
    GetDictTuple(hv_ExampleInternals, "window_legend", &hv_WindowHandleLegend);
    SetDictTuple(hv_WindowHandleDict, "segmentation_image_result", hv_WindowHandleImages.TupleConcat(hv_WindowHandleLegend));
    //Display results.
    dev_display_dl_data(hv_Sample, hv_Result, hv_Info, "segmentation_image_result",
        hv_GenParam, hv_WindowHandleDict);
    //
    //Display area.
    GetDictTuple(hv_WindowHandleDict, "segmentation_image_result", &hv_WindowHandles);
    HDevWindowStack::SetActive(HTuple(hv_WindowHandles[0]));
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), ((HTuple(hv_ClassNames[1]) + "\narea: ") + hv_Area) + "px",
            "image", hv_Row - 10, hv_Column + 10, "black", HTuple(), HTuple());
    if (HDevWindowStack::IsOpen())
        DispText(HDevWindowStack::GetActive(), "Segmented defect regions", "window", "top",
            "left", "black", HTuple(), HTuple());
    FlushBuffer(HTuple(hv_WindowHandles[0]));
    //
    return;
}
...
void get_inference_images(HTuple hv_ImageDir, HTuple* hv_ImageFiles)
{
    ...
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/magnesium/crack/pill_magnesium_crack_036";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/mint/good/pill_mint_good_165";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/ginseng/crack/pill_ginseng_crack_004";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/magnesium/good/pill_magnesium_good_066";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/ginseng/contamination/pill_ginseng_contamination_244";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/mint/contamination/pill_mint_contamination_076";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/magnesium/contamination/pill_magnesium_contamination_011";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/ginseng/good/pill_ginseng_good_042";
    (*hv_ImageFiles)[(*hv_ImageFiles).TupleLength()] = hv_ImageDir + "/mint/crack/pill_mint_crack_121";
    return;
}
...
void action()
{
    ...
    HTuple  hv_ImageDir, hv_ExampleDataDir, hv_PreprocessParamFileName;
    HTuple  hv_RetrainedModelFileName, hv_DataDirectory, hv_ClassNames;
    HTuple  hv_ClassIDs, hv_BatchSizeInference, hv_DLModelHandle;
    HTuple  hv_DLPreprocessParam, hv_WindowHandleDict, hv_DatasetInfo;
    HTuple  hv_GenParamDisplay, hv_ImageFiles, hv_BatchIndex;
    HTuple  hv_Batch, hv_DLSampleBatch, hv_DLResultBatch, hv_SampleIndex;
    HTuple  hv_WindowHandles, hv_Areas, hv_ClassIndex, hv_Area;
    ...
    if (0 != hv_ShowExampleScreens)
    {
        ...
        dev_example_init(hv_ShowExampleScreens, &hv_ExampleInternals);
        //
        ...
        //
        //Explain inference.
        dev_display_screen_example_images(hv_ExampleInternals, hv_UsePretrainedModel);
        // stop(...); only in hdevelop
        //
        //Explain inference steps.
        dev_display_screen_inference_step_1(hv_ExampleInternals);
        // stop(...); only in hdevelop
        dev_display_screen_inference_step_2(hv_ExampleInternals);
        // stop(...); only in hdevelop
        dev_display_screen_inference_step_3(hv_ExampleInternals);
        // stop(...); only in hdevelop
        dev_display_screen_inference_step_4(hv_ExampleInternals);
        // stop(...); only in hdevelop
        //
        ...
        dev_display_screen_ready_to_infer(hv_ExampleInternals);
        // stop(...); only in hdevelop
        //
    ...
    get_inference_images(hv_ImageDir, &hv_ImageFiles);
    //
    //Loop over all images in batches of size BatchSizeInference for inference.
    {
        HTuple end_val149 = (((hv_ImageFiles.TupleLength()) / (hv_BatchSizeInference.TupleReal())).TupleFloor()) - 1;
        HTuple step_val149 = 1;
        for (hv_BatchIndex = 0; hv_BatchIndex.Continue(end_val149, step_val149); hv_BatchIndex += step_val149)
        {
            //
            ...
            ApplyDlModel(hv_DLModelHandle, hv_DLSampleBatch, (HTuple("segmentation_image").Append("segmentation_confidence")),
                &hv_DLResultBatch);
            //
            //Postprocessing and visualization.
            //Loop over each sample in the batch.
            {
                HTuple end_val167 = hv_BatchSizeInference - 1;
                HTuple step_val167 = 1;
                for (hv_SampleIndex = 0; hv_SampleIndex.Continue(end_val167, step_val167); hv_SampleIndex += step_val167)
                {
                    //
                    //Get image.
                    GetDictObject(&ho_Image, HTuple(hv_DLSampleBatch[hv_SampleIndex]), "image");
                    //Get result image.
                    GetDictObject(&ho_SegmentationImage, HTuple(hv_DLResultBatch[hv_SampleIndex]),
                        "segmentation_image");
                    //
                    ...
                    //
                    //Display results.
                    dev_display_dl_data(HTuple(hv_DLSampleBatch[hv_SampleIndex]), HTuple(hv_DLResultBatch[hv_SampleIndex]),
                        hv_DatasetInfo, "segmentation_image_result", hv_GenParamDisplay, hv_WindowHandleDict);
                    GetDictTuple(hv_WindowHandleDict, "segmentation_image_result", &hv_WindowHandles);
                    HDevWindowStack::SetActive(HTuple(hv_WindowHandles[0]));
                    //
                            ...
                            if (0 != (int(HTuple(hv_Areas[hv_ClassIndex]) > 0)))
                            {
                                ...
                                AreaCenter(ho_ConnectedRegions, &hv_Area, &hv_Row, &hv_Column);
                                {
                                    ...
                                    for (hv_ConnectIndex = 0; hv_ConnectIndex.Continue(end_val196, step_val196); hv_ConnectIndex += step_val196)
                                    {
                                        ...
                                        if (HDevWindowStack::IsOpen())
                                            DispText(HDevWindowStack::GetActive(), ((HTuple(hv_ClassNames[hv_ClassIndex]) + "\narea: ") + HTuple(hv_Area[hv_ConnectIndex])) + "px",
                                                "image", HTuple(hv_Row[hv_ConnectIndex]) - 10, HTuple(hv_Column[hv_ConnectIndex]) + 10,
                                                "black", HTuple(), HTuple());
                                    }
                                }
                            }
                    ...
                    dev_display_ok_nok(hv_Areas, HTuple(hv_WindowHandles[0]));
                    //
                    // stop(...); only in hdevelop
                }
            }
