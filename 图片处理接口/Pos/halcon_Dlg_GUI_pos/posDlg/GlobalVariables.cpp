...
namespace UCI {
	...
	void initOptionsMap(OptionsMap& o) {
		...
		o[CST_PosProcID] << Option(CST_PosProcID, 1, 1, 200000,                             // 当前的项目编号
			"project ID", false);	

		//o["当前属性ID"] << Option("当前属性ID", 1, 1, 200000,                         // 当前的在属性列表中显示的ID
		//	"当前属性ID", true);

		o[CST_PosImageWHR] << Option("dRadio", 600.0f/800.0f, 0.5f, 2.0f,                            // 图像窗口比例
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////
		/// 标定文件名设置
		o[CST_PosImageWidth] << Option("ORGwidth", 2592, 200, INT_MAX,
			"width [100,2592] pixel");                                                           // "图像宽度"
		o[CST_PosImageHeigh] << Option("ORGheight", 2073, 200, INT_MAX,
	...
	void stElement::initOptionMapMATCH(OptionsMap & o)
	{
		...
		o[CST_PosNumLevels] << Option("NumLevels", "auto",                                              // FindShapeModleMinScore
...
//			"CreateModle double AngleStart RAD Default value: -0.39 Suggested values: -3.14, -1.57, -0.78, -0.39, -0.20, 0.0 ");
		...
		o[CST_PosAngleExtend] << Option("AngleExtend", 20.0f, 360.0f, 0.0f,                              // CreateShapeModel double AngleExtend
			"Rotation Range Default values: 20 Suggested values: 360, 0.0 ");

		o[CST_PosStep] << Option("AngleStep", "auto",                                               // CreateShapeModel double AngleExtend
		...
		o[CST_PosContrast] << Option("Contrast", "auto",                                         // FindShapeModleMinScore
		...
		o[CST_PosMinScroe] << Option("MinScore", 0.5f, 0.00f, 1.0f,                                      // FindShapeModleMinScore
			"Default value: 0.5 Suggested values: 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0");

		o[CST_PosNumMatches] << Option("NumMatches", 1, 0, 20,                                              // FindShapeModleMinScore
			"Default value: 1 Suggested values: 0, 1, 2, 3, 4, 5, 10, 20");
		
		o[CST_PosNumMatchesLimit] << Option("NumMatchesLimit", 2, 0, 99999999,                                              // FindShapeModleMinScore
		...
		o[CST_PosModeType] << Option("ModelTypes", 0, 0, 1,                                              // FindShapeModleMinScore
			"Match type shape or NCC matching  Default value: 0  List of values: 0, 1");
		...
		o[CST_PosCheckScore] << Option("RES_FindScore", "",                                         // FindShapeModleMinScore
			CST_PosCheckScore);
	...
	OptionsMap* stElement::initOptionMapElemnet(eELEMENT_TYPE t, int iPID)
	{
		...
		// 初始化一些公共变量                                                   
		o[CST_PosProcID] << Option(CST_PosProcID, iPID, 1, INT_MAX,                         // 当前组的所属的项目编号
			"Current Project ID", false);

		o[CST_PosEleType] << Option(CST_PosEleType, t, 0, ELEMNET_END - 1,                    // 当前参数元素类型
			"Current Elemnet Type",false);
		
		o[CST_PosEleName] << Option(CST_PosEleName, name,                                     // 元素的名称
			"Element Name", true);	
		...
		o[CST_GLBPACommandLine] << Option(CST_GLBPACommandLine, "none", "Command Macro", true);                           // 元素的命令宏
		o[CST_GLBPAEleNote] << Option(CST_GLBPAEleNote, "note", "Element Note", true);                        // 元素的备注
