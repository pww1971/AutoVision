	...
	:CMFCPropertyGridProperty(_T("tmp"))
{
	...
	case PROPRETY_GLOBAL:
		this->SetName(CST_PosWord20);
		initGobleProperty(om);
		break;
	case PROPRETY_ELEMENT:
		this->SetName(CST_PosWord21);
					...
					if (show == true) {
						...
						if (type == "spin") {
							...
							if (name == "相机协议") {
								...
								CString sCamInterFace = UCI::OPTIONS["相机协议"];
								HTuple CamInfo;
								HTuple CamValues;
			...
			if (it->second.getIdx() == idx) {
				...
				if (show == true) {
					...
					if (type == "spin") {
						...
						else if (name == CST_GLBPACommandLine)
						{
							pProp->AllowEdit(TRUE);
						}
						else if (name ==CST_GLBPAEleNote)
						{
							pProp->AllowEdit(TRUE);
						}
						this->AddSubItem(pProp);
			...
			if (it->second.getIdx() == idx) {
				...
				if (show == true) {
					...
					if (type == "spin") {
						...
						else if (name == "USE_LINE1_Parament_Name" // 二直线测量
			...
			if (it->second.getIdx() == idx) {
				...
				if (show == true) {
					...
					if (type == "spin") {
						...
						else if (name == "引用元素名称1" // 二直线测量
							|| name == "引用元素名称2")
						{
							insertCurrentPosCanUseElement(pProp, om);   // 查找当前的所有可输出元素	
						}
						else if (name == "引用元素1参数名") {
							pProp->SetData(USE_Distance1_Parament_Name_ID);
						}
						else if(name == "引用元素2参数名") {   // 要选择的元素输出属性
			...
			if (it->second.getIdx() == idx) {
				...
				if (show == true) {
					...
					if (type == "spin") {
						...
						else if (name == "输出元素名称")		// RESULT_ELEMENT 元素					
						{
							insertCurrentPosCanUseElement(pProp, om);   // 查找当前的所有可输出元素
						}
						else if (name == "输出元素的参数") {   // 要选择的元素输出属性
			...
			if (it->second.getIdx() == idx) {
				...
				if (show == true) {
					...
					if (type == "spin") {
						...
						else if (name == "校正元素的ID X") {
							pProp->SetData(OUT_Element_Name_IdentifyX);
						}
						else if (name == "校正元素的ID Y") {
						...
						if (name == "输出的标准值")
					   {
						...
						else if (name == "校正元素 X")		// RESULT_ELEMENT 元素					
						{
							insertCurrentPosCanUseElement(pProp, om);   // 查找当前的所有可输出元素	
						}
						else if (name == "校正元素的参数 X") {   // 要选择的元素输出属性
							pProp->SetData(OUT_Element_Name_IDX);
						}
						else if (name == "校正元素 Y")		// RESULT_ELEMENT 元素					
						{
							insertCurrentPosCanUseElement(pProp, om);   // 查找当前的所有可输出元素	
						}
						else if (name == "校正元素的参数 Y") {   // 要选择的元素输出属性
...
{
	int emtype = om[CST_PosEleType];
	CString omName = om[CST_PosEleName];
