...
BOOL ST_MYDISP::MeasureOneDistance(CList <UCI::OptionsMap, UCI::OptionsMap>& eleList,UCI::OptionsMap * o, HImage & hImg, HXLDCont& Cont)
{
	try
	{
		...
		int etype1 = (*oDIS1)[CST_PosEleType];
		int etype2 = (*oDIS2)[CST_PosEleType];
...
BOOL ST_MYDISP::MeasureOneMATCH(UCI::OptionsMap * o, HImage & hImg, HXLDCont &Cont)
{
	try {
		...
		//BOOL bLoadROI = (*o)["匹配模板 OK"];

		//if (bLoadROI == FALSE) {
		//	write_message(420, 20, "没有加载模板", true);
		//	return FALSE;
		...
		//	//	MyLog(stRegion);
		//	//	AfxMessageBox("MeasureOneMATCH() ROI　区域文件不存在！ ");
		//	//	return FALSE;
		//	//}

		//	//if (GetFileAttributes(stModel) == -1) { // 文件不存在
		//	//	MyLog(stModel);
		//	//	AfxMessageBox("MeasureOneMATCH() ROI　模板文件不存在！ ");
		//	//	return FALSE;
		//	//}

		//	//hv_Model->hv_Model.ReadShapeModel(CT2A(stModel).m_psz);       // 读出模板文件
		//	//hv_Model->ho_ModelRegion.ReadRegion(CT2A(stRegion));          // 读出匹配区域文件
		...
		(*o)[CST_PosCheckScore] = Score;

		if (Row.Length() < 1 ) {
			write_message(420, 20, CST_PosWord3, true);
		...
		/*hv_Model->ho_ModelRegion.AreaCenter(&Row0, &Col0);
	...
	try {
		...
		while (pos != NULL) {
			...
			if (!this->MeasureOneElement(*UCI::ELE.EleList,o, hImg, Contours)) {				
				...
				CString eleName = (*o)[CST_PosEleName];
