...
{ 
	...
	{
		MessageBox(0,"（Pos.dll）Positioning failed", "System prompts", MB_YESNO | MB_ICONQUESTION) ;
		return false;
	}
	return true;
}
...
void DesignDllRelease(CTpltParam& pa)
{
	...
	if (pa.m_dllPa.ini == 1)
	{
		...
		{
			MessageBox(0,"delete（Pos.dll）Fail", "System prompts", MB_YESNO | MB_ICONQUESTION) ;
			return;
		}
		pa.m_dllPa.sel = NULL;
		pa.m_dllPa.ini = 0;
...
void DesignDllWriteFile(CTpltParam& pa)
{
	...
	if (pa.m_dllPa.ini == 1)
	{
		...
		{
			MessageBox(0,"（Pos.dll）write file fail", "System prompts", MB_YESNO | MB_ICONQUESTION) ;
			return;
		}
	}
