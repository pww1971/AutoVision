...
BEGIN_MESSAGE_MAP(CDesign_dlg_posDlg, CDialogEx)
	...
	ON_WM_MOUSEWHEEL()
	ON_WM_TIMER()
//	ON_EN_CHANGE(IDC_MFCEDITBROWSE1, &CDesign_dlg_posDlg::OnEnChangeMfceditbrowse1)
END_MESSAGE_MAP()
...
//	((CComboBox*)GetDlgItem(IDC_COMBO1))->SetCurSel(0);
	...
	if (!m_wndProperties.Create("属性", this, CRect(rect.right + 2, rect.top, rect2.right - 10, rect.bottom), TRUE, ID_VIEW_PROPERTIESWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_RIGHT | CBRS_FLOAT_MULTI))
	{
		TRACE0("未能创建“属性”窗口\n");
		return FALSE; // 未能创建
	}
...
void CDesign_dlg_posDlg::FillProjectTreeView()
{
	...
	HTREEITEM hRoot = m_ProcTree.InsertItem(_T("测量项目"), 0, 0);
	...
	try
	{
		...
		if(NULL != UCI::ELE.EleList)
		{
			...
			while (pos != NULL) 
			{
				...
				CString name = (*o)["元素名称"].getcurrentValue();
				int id = atoi((*o)["项目编号"].getcurrentValue().GetBuffer(0));
...
void CDesign_dlg_posDlg::OnBnClickedBtOpen()
{
	...
	{
		CFileDialog fdlg1(true, NULL, getExePath().c_str(), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "open file (*.dxf)|*.dxf||");
		if (fdlg1.DoModal() == IDOK)
		{
			CString str;
			str.Format("匹配 %d", selectid);
			UCI::OptionsMap* om = UCI::ELE.GetOptionsMapByEleName(*UCI::ELE.EleList, str);
			UCI::pDis->CreatDxfModel(om, fdlg1.GetPathName());
		}
	}
	else
	{
		FillProjectTreeView();
		AfxMessageBox("请选择匹配方案");
	}
