...
extern "C" PROCLIB_API bool DesignDllDxfModel(string dxffile, CTpltParam & pa, cv::RotatedRect & rrt);
bool DesignDllDxfModel(string dxffile, CTpltParam& pa, cv::RotatedRect& rrt)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	if (pa.m_dllPa.p != NULL)
		return ((checkPatch*)pa.m_dllPa.p)->creatDxfModel(dxffile, pa, rrt);
	return false;
}
