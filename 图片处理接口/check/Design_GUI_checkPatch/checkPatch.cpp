...
void S_CheckParam::writeParam(string file)
{
	...
	int len = m_bgAlg.size();
	fwrite(&len, sizeof(int), 1, fp);
	fwrite(m_bgAlg.c_str(), len,1, fp);
	fclose(fp);
}
...
{
	m_szRet ="";
	bool bColor = false;//原始图是彩色的
	if (m_myParam.m_iHisType != 4)
	{
		if (src.channels() > 1)
		{
			bColor = true;
	...
	{
		resize(src, src, cv::Size(src.rows * m_myParam.m_tpltPa.m_scale, src.cols * m_myParam.m_tpltPa.m_scale));
	}
	if (m_myParam.m_iHisType < 2)
	{
		cv::Mat m = cv::Mat(src.rows, src.cols, src.type(), Scalar(0));
		if (m_myParam.m_iHisType == 0)
		{
			lbp(src, m);
		}
		else// if (m_myParam.m_iHisType == 1)
		{
			elbp(src, m);
		}
		calTexture(m_myParam.m_N, m_myParam.m_tpltPa.sThdMin[2], m_myParam.m_tpltPa.sThdMax[2], m, src);
	}
	else if (m_myParam.m_iHisType == 2)//
	{
		blur(src, src, cv::Size(9, 9));
		Canny(src, src, m_myParam.m_tpltPa.sThdMin[2], m_myParam.m_tpltPa.sThdMax[2]);
		if (bColor)//还原原始图的彩色状态
			cv::cvtColor(src, src, cv::COLOR_GRAY2RGB);
	}
	else if (m_myParam.m_iHisType == 3)//
	{
		if(src.channels()==1 && m_bgImg.channels()==3)
			cv::cvtColor(m_bgImg, m_bgImg, cv::COLOR_RGB2GRAY);
		absdiff(src , m_bgImg,src);
	}
	else //== 4
	{
		if (src.channels() == 1 && m_bgImg.channels() == 3)
...
//	readParamFromTxt(file + "\\checkParam2.cfg",m_myParam);
	...
	m_bgImg = cv::imread(file +"/bg.bmp", cv::IMREAD_UNCHANGED);
}
...
//	writeParamToTxt(file + "\\checkParam2.cfg",m_myParam);
	...
	if (!m_bgImg.empty())
		cv::imwrite(file + "/bg.bmp", m_bgImg);
}
