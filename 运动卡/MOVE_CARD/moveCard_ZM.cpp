...
bool CMoveCard::ini(string linkAdd)
{
	...
	//	"回零触发模式","回零模式","导承","回零速度","手动速度"};
	char* szContent[] = { CST_AxisID,CST_NegLimit,CST_PosLimit,CST_AxisPulseMode, CST_SCurve,\
	CST_PulseEquivalent, CST_Acc, CST_Dec, CST_Speed, CST_ZeroReverSpeed, CST_StartSpeed, CST_AxisNegLimit, CST_AxisPosLimit\
	CST_ZeroTriggerMode,CST_ZeroMode, CST_BearGuide, CST_ZeroSpeed, CST_ManualSpeed };
...
bool CMoveCard::getParam(vector<string>& vctPa)
{
	...
	for (int mtid=0;mtid<m_mtNum;mtid++)
	{
		...
		for (int i=0;i<commandLen;i++)
		{
			if (5 == i)
			{
				val = m_fSpeed[mtid];//检测速度 不用当前的速度单位
			}
			else
			{
...
bool CMoveCard::setParam(string szPa)
{
	...
	try
	{
		if (""==szPa)//加载默认参数
		{
			int zeroModel = m_vctDefaultData[12];//回零触发模式
			for (int mtid=0;mtid<m_mtNum;mtid++)
			{
		...
		for (int mtid=0;mtid<m_mtNum;mtid++)
		{
			...
			if (len>15)
			{	
				zeroModel = atoi(command[12].c_str());
				m_fZeroMode[mtid] = atoi(command[13].c_str());
			}
			ZAux_SetDatumin(m_handle, mtid, zeroModel);
