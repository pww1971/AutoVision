...
BOOL CCameraCPP::Connect(CString str)
{
	...
	CString interfaceName = UCI::OPTIONS["相机协议"];
	CString deviceName = UCI::OPTIONS["相机名称"];
	try {
		if (str != "NotFiles") {
		...
		UCI::OPTIONS["图像宽度"] = hv_Value.I();


		hv_Value = this->m_pFG->GetFramegrabberParam("image_height");
		UCI::OPTIONS["图像高度"] = hv_Value.I();
...
{	
	try 
	{
		...
		if (isUseStorePic) {   // 加载一个测试图		
			...
			ReadImage(&ImageAlreadyMapped, HTuple(hv_ImageFiles[sid++ % hv_ImageFiles.TupleLength().I()]));

			GrabImage = ImageAlreadyMapped;
		}
		else
...
void CCameraCPP::GetLastBufferImage(HalconCpp::HImage* img)
{
	//unsigned       idx;
	// get the new image buffer
