...
namespace UCI {
	...
	void initOptionsMap(OptionsMap& o) {
		...
		o["软触发定时"] << Option("iSoftTimeCamCapture", 20, 10, 2000,                         // 软触发的定时时间
			"In soft trigger mode,Camera capture one picture time [10,2000] defautl 20ms");   

		o["触发方式"] << Option(" trigger_mode", 0, 0, 2,                                      // 触发方式
			"trigger mode 0: continues 1: soft trigger 2: hard trigger", 
			true,on_camera_para);

		o["相机协议"] << Option("CAMinterfaceName","DirectShow",                      // 接口名称 File, DirectShow ...
			"Camer interface: MindVision12_X64 or DirectShow");  

		o["相机名称"] << Option("CAMDeviceName", "[0] Integrated Camera",                                    // 当前选择的相机名称
			"Camera DeviceName: [0] Integrated Camera or CM300 ");  

		o["相像保存格式"] << Option("PictureSaveFormat", "png",                              // 图像保存格式
			"Picture Save Format",false);

		//o["CAMCurGroupID"] << Option("CAMCurGroupID", 1, 1, 10,                               // 当前的相机参数组号	
		...
		o["转换矩阵成功"] << Option("GLO_HomMat2D_OK", 0, 0, 1,                                // 找到模型，并建立了转换矩阵了。
			"Fine model and already creat HomMat2D Is OK", true);
		...
		o["图像高度"] << Option("ORGheight", 1944, 200, INT_MAX,
			"height [100,1944] pixel");
	...
	//		"Cali One Pixel Distane [0.00000001, 1.0] m default: 0.00013");
		...
		o["单个像素长度mm"] << Option("OnePixelScalemm", 1.0f, 0.0001f, FLT_MAX,               // 最终一个像素代表的尺寸
			"OnePixel Scale mm [0.0001, FLT_MAX] degree default: 0.030");

		o["结果保留小数点位数"] << Option("RetPos", 2, 0, 6,          
			"结果保留小数点位数 默认2");

	
		///////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// 其它杂项
		o["使用图片文件夹"] << Option("使用图片文件夹", 0, 0, 1,                                                         
			"Test Use prestored Picture", true);

		o["图片文件夹名称"] << Option("图片文件夹名称", "0",                                                 
			"图片文件夹名称 ", true);
		...
		o["UserID"] << Option("UserID", 1, 1, INT_MAX,                                                            // 
			"Current logged User iD ", false); 

		o["UserLogName"] << Option("UserLogName", "NoUser",                                                      // 用户登录姓名
			"UserLogName ", true);

		o["UserLogDateTime"] << Option("UserLogDateTime", "20170816 08:00",                                       // 用户登录时间
			"UserLogDateTime ", true);
		...
		o["AutoMeasureDelayTime"] << Option("AutoMeasureDelayTime", 50, 20, 2000,                              // 
			"Auto Measure Delay Time ms", true);

		o["首张延时"] << Option("首张延时", 600, 10, 5000,                                                      // 
			"照相机拍第一张时的延时",
			true);
	...
	void stElement::initOptionMapElemnetCamera(OptionsMap & o)
	{
		...
		(o)["图像采样超时"] << Option("grab_timeout", 300, 50, 100000,                    // 收集一副图像超时设置 ms
			"grab_timeout [50, 100000] ms default 50ms",
			true);
		(o)["爆光时间"] << Option("shutter", 20000, 50, 1000000,                           // 爆光时间 us
			"shutter time  [50, 1000000] μs default 20000 20ms",
			true );
		(o)["对比度"] << Option("contrast", 100, 1, 200,                                // 对比度
			"LUT diagram contrast [1, 200] defalut 100",
			true );
		(o)["gamma"] << Option("gamma", 100, 1, 1000,                                     // gamma
			"LUT diagram gamma [1,1000] default 100",
			true );
		(o)["爆光模式"] << Option("exposure_mode", 0, 0, 1,                              // 爆光方式
			"exposure mode 0: manual, 1: automatic",
			true );
		(o)["帧速率"] << Option("frame_speed", 0, 0, 2,                                  // 帧速率
			"frame speed 0: low speed, 1: mid speed, 2: high speed",
			true );
		(o)["anti_flick"] << Option("anti_flick", 0, 0, 2,                                // 抗屏闪
			"anti_flick 0：not use, 1: anti 50HZ, 2: anti 60HZ",
			false );
		(o)["resolution"] << Option("resolution", 0, 0, 255,                              // 分辨率
			"resolution [0, 255] default 0", false );
		(o)["resolution_mod"] << Option("resolution_mod", "Normal",                       // 分辨方式 Normal BINSUM2X BINAVR2X Uknown
			"resolution mod [Normal,BINSUM2X BINAVR2X Uknown] default Normal",
			false );
		(o)["offsetX"] << Option("offsetX", 0, 0, 2592,                                   // 
			"offsetX [0, 2592] default 0",
			true );                             // 
		(o)["offsetY"] << Option("offsetY", 0, 0, 1944,
			"offsetY [0, 1944] default 0",
			true );                             // 
		(o)["采样宽度"] << Option("width", 2592, 100, INT_MAX,
			"width [100,2592] pixel",
			true );                                 // 
		(o)["采样高度"] << Option("height", 1944, 100, INT_MAX,
			"height [100,1944] pixel",
			true );                                // 

		(o)["色温"] << Option("color_temperature", 0, 0, 3,                                 // 色温
			"color temperature 0: auto 1: D65 2: 5500K (pure white) 3: cloudy sky in door",
			true );
		(o)["红色增益"] << Option("red_gain", 1.00f, 0.10f, 4.0f,                             // 红色增益
			"red gain [0.0, 3.99]",
			true );
		(o)["绿色增益"] << Option("green_gain", 1.00f, 0.10f, 4.0f,                         // 绿色增益
			"green gain [0.0, 3.99]",
			true );
		(o)["蓝色增益"] << Option("blue_gain", 1.00f, 0.10f, 4.0f,                           // 蓝色增益
			"blue gain [0.0, 3.99]",
			true );
		(o)["饱和度"] << Option("saturation", 0, 0, 200,                                  // 饱和度
			"saturation [0, 200]",
			true );
		(o)["图像增益"] << Option("gain", 1.25f, 1.25f, 50.0f,                                   // 增益
			"gain [1.25, 50.0]",
			true );
		(o)["色彩空间"] << Option("color_space", "Gray",                                  // 色彩空间 BGR24
			"color space [Gray, BGR24]",
			true );
		(o)["采样延时"] << Option("采样延时", 0, 0, 2000,                                  
			"采样前延时 [0, 1200] ms", true);
	}
	...
	void stElement::initOptionMapMATCH(OptionsMap & o)
	{
		...
		o["起始角弧度"] << Option("AngleStart", -0.39f, -3.14f, 3.14f,                             // CreateShapeModel double AngleStart
			"CreateShapeModle double AngleStart RAD Default value: -0.39 Suggested values: -3.14, -1.57, -0.78, -0.39, -0.20, 0.0 ");
		
		o["旋转范围"] << Option("AngleExtend", 0.78f, 0.0f, 6.29f,                               // CreateShapeModel double AngleExtend
			"CreateShapeModle double AngleExtend Default value: 0.78 Suggested values: 6.29, 3.14, 1.57, 0.78, 0.39, 0.0 ");

		o["步数"] << Option("AngleStep", "auto",                                               // CreateShapeModel double AngleExtend
			"CreateShapeModle double AngleStep Default value: auto Suggested values: 0.0175, 0.0349, 0.0524, 0.0698, 0.0873 Restriction: (AngleStep >= 0) && (AngleStep <= (pi / 16))");
		...
		o["CreateModleContrast"] << Option("Contrast", "auto",                                         // FindShapeModleMinScore
		...
		o["相似度"] << Option("MinScore", 0.85f, 0.30f, 1.0f,                                      // FindShapeModleMinScore
			"MinScore  Default value: 0.5 Suggested values: 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0");
		...
		o["匹配模型"] << Option("ModleTypes", 0, 0, 1,                                              // FindShapeModleMinScore
