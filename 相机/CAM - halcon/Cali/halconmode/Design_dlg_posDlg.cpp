...
BOOL CDesign_dlg_posDlg::OnInitDialog()
{
	...
	{
#ifdef HCAM
		int camN = ((checkPatch*)m_pCheck)->HCameraDrv::ini();
#else
		...
		for (int i = 0; i < camN; i++)
		{
			...
			AddOneElementToProjectTree(o);   // 3. 添加到项目树中去
...
void CDesign_dlg_posDlg::OnBnClickedBtInfo()
{
	...
	if (selectid > 0 && selectid < 10)
		UCI::pDis->DisOnButCaliShowInfo(selectid); // 显示当前的标定信息
	else
	{
		FillProjectTreeView();
		AfxMessageBox("请选择相机");
...
#ifdef HCAM
		vector<HImage> vm;
		if (-1 != ((checkPatch*)m_pCheck)->HCameraDrv::grapImage(id - 1, vm))
		{
...
#else
		vector<cv::Mat> vm;
		if (-1 != ((checkPatch*)m_pCheck)->CameraDrv::grapImage(id - 1, vm))
		{
			UCI::OPTIONS["图像宽度"] = vm[0].cols;
			UCI::OPTIONS["图像高度"] = vm[0].rows;
			UCI::OPTIONS["图像宽长比"] = (double)vm[0].rows / vm[0].cols;
