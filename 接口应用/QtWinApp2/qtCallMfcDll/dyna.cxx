...
#elif _WIN32
		strcat(ptlibPath, DIR_TOKENISER "C:\\Windows\\System32");
#endif
	}
...
bool DynaLink::InternalOpen(const char * dir, const char *name)
{
	...
	if (strlen(path) == 0) {
		//    printf(1, "DynaLink", "dir '" << (dir != NULL ? dir : "(NULL)")
...
# endif /* UNICODE */
	...
	if (m_hDLL == NULL) {
		...
		if (err != NULL)
            printf( "DynaLink", "dlopen error %s" , err);
		else
            printf( "DynaLink", "dlopen error loading %s " , path);
#else /* _WIN32 */
         printf( "DynaLink", "Error loading %s" , path);
#endif /* _WIN32 */
		return false;
	}
...
#ifdef _WIN32
# ifdef UNICODE
	GetModuleFileName(m_hDLL, LPWSTR(path), sizeof(path));
#else
	GetModuleFileName(m_hDLL, path, sizeof(path));
#endif
#endif

	// printf(1, "DynaLink", "Successfully loaded '" << path << "'");
	return true;
}
...
void DynaLink::Close()
{
	if (m_hDLL != NULL) {
#ifdef _WIN32
		//没办法注释掉。由于dll里面开的线程不能停，因此线程开的时候关闭dll会卡死
		//要么采用静态链接dll的方式
		//FreeLibrary(m_hDLL);
#else
		dlclose(m_hDLL);
...
bool DynaLink::GetFunction(const char * name, Function & func)
{
...
#ifdef UNICODE
	USES_CONVERSION;
	FARPROC p = GetProcAddress(m_hDLL, (LPCSTR)(name));
#else
	FARPROC p = GetProcAddress(m_hDLL, name);
#endif /* UNICODE */
	if (p == NULL) {
		//  printf(1, "DynaLink", "Error linking function " << name << ", error=" << GetLastError());
		return false;
	}
	...
	if (p == NULL) {
		//  printf(1, "DynaLink", "Error linking function " << name << ", error=" << dlerror());
		return false;
	}
	func = (Function &)p;
